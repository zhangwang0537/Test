...
namespace matlab {
    namespace engine {
        ...
        inline std::unique_ptr<MATLABEngine> startMATLAB(const std::vector<String>& options) {
            return startMATLABAsync(options).get();
        }

        inline FutureResult<std::unique_ptr<MATLABEngine>> startMATLABAsync(const std::vector<String>& options) {
            initSession();
            auto startMATLABType = [options]() {
                std::vector<char16_t*> options_v(options.size());
                std::transform(options.begin(), options.end(), options_v.begin(), [](const String& option){ return const_cast<char16_t*>(option.c_str());  });
                bool errFlag = false;
                uint64_t matlab = engine_create_out_of_process_matlab(options_v.data(), options_v.size(), &errFlag);
                if (errFlag) {
        ...
        inline std::vector<String> findMATLAB() {
            initSession();
            char16_t** names;
            size_t size = engine_find_shared_matlab(&names);
            std::vector<String> names_v(size);
            for (size_t i = 0; i < size; i++) {
                names_v[i] = names[i];
            }
            engine_destroy_names(names, size);
            return names_v;
        }

        inline FutureResult<std::vector<String>> findMATLABAsync() {
            std::future<std::vector<String> > stdFuture = std::async(std::launch::async, findMATLAB);
            FutureResult<std::vector<String>> future(std::move(stdFuture), std::make_shared<TaskReference>(&cancelFind));
            return future;
        }
        ...
        inline std::unique_ptr<MATLABEngine> connectMATLAB(const String& name) {
            return connectMATLABAsync(name).get();
        }
        
        inline FutureResult<std::unique_ptr<MATLABEngine> > connectMATLABAsync() {
            std::vector<String> engines = findMATLAB();
            if (!engines.empty()) {
                return connectMATLABAsync(engines[0]);
            }
            else {
                std::vector<String> options;
                String option1 = convertUTF8StringToUTF16String("-r");
                String option2 = convertUTF8StringToUTF16String("matlab.engine.shareEngine");
        ...
        inline FutureResult<std::unique_ptr<MATLABEngine>> connectMATLABAsync(const String& name) {
            initSession();
            auto connectMATLABType = [name]() {
                bool errFlag = false;
                uint64_t matlab = engine_attach_shared_matlab(name.c_str(), &errFlag);
                if (errFlag) {
